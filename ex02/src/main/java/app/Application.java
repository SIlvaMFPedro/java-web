...
public class Application {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String url = getUrls(request.get(0));
    ...
    private static String getUrls(String line){
    ...
    private static Map<String, String> getBodyParams(List<String> request){
        ...
        Arrays.stream(request.get(request.size() - 1).split("&")).map(bp -> bp.split("=")).forEach(bpKvp -> bodyParams.put(bpKvp[0], bpKvp[1]));
        return bodyParams;
    }

    private static Map<String, String> getHeaders(List<String> request) {
        Map<String, String> headers = new LinkedHashMap<>();
        request.stream().skip(1).filter(h -> h.contains(": ")).map(h -> h.split(": ")).forEach(hKvp -> headers.put(hKvp[0], hKvp[1]));
        return headers;
    }
}